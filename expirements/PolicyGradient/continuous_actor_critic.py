...
class ActorCritic(nn.Module):
  ...
  def choose_action(self, obs):
    ...
    l_probs = dist.log_prob(probs).to('cpu')
    return action, l_probs.detach().numpy()

  # state, reward, nstate, log_prbs, done
  def store(self, s, r, ns, lp, d):
    self.memory.append((s, r, ns, lp, d))
  ...
  def train(self):
    ...
    l_probs = torch.tensor([x[3] for x in self.memory])
    ...
    delta = rewards + self.gamma*nvalue*dones - value
    print(nvalue.shape, rewards.shape, dones.shape)

    #print( delta.shape, l_probs.shape)
    loss = -l_probs * delta  + delta**2
...
while time_step <= max_training_timesteps:
  ...
  for t in range(1, max_ep_len+1):
    ...
    action, l_probs = agent.choose_action(torch.from_numpy(state))
    #action, l_probs = env.action_space.sample()
    #action = action.detach().numpy

    nstate, reward, done, _ = env.step(action)
    agent.store(state, reward, nstate, l_probs, done)
